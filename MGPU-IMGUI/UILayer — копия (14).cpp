...
void UILayer::SetTexture()
{
    ...
    uiNavigationPicturesDescriptors[0] = device->AllocateDescriptors(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);

    for (uint16_t i = 0; i < 63; i++)
    {
        skillPicturesDescriptors[i] = device->AllocateDescriptors(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
    }
    ...
    for (uint16_t i = 0; i < 63; i++)
    {
        ...
        auto tempTex = GTexture::LoadTextureFromFile(wideStr, cmdList);

        tempTex->CreateShaderResourceView(&srvDesc, &skillPicturesDescriptors[i], 0);

        assets->AddTexture(tempTex);
    }
    ...
    {
        std::wstring wideStr(NAVIGATION_ICONS_PLACEMENTS[0], NAVIGATION_ICONS_PLACEMENTS[0] + strlen(NAVIGATION_ICONS_PLACEMENTS[0]));     
        auto tempTex = GTexture::LoadTextureFromFile(wideStr, cmdList);

        tempTex->CreateShaderResourceView(&srvDesc, &uiNavigationPicturesDescriptors[0], 0);
    ...
    auto tempTex = GTexture::LoadTextureFromFile(wideStr, cmdList);
    tempTex->CreateShaderResourceView(&srvDesc, &uiNavigation, 0);
    assets->AddTexture(tempTex);

    wideStr = std::wstring(UI_MAP, UI_MAP + strlen(UI_MAP));
    tempTex = GTexture::LoadTextureFromFile(wideStr, cmdList);
    tempTex->CreateShaderResourceView(&srvDesc, &uiMapDesc, 0);
    assets->AddTexture(tempTex);
